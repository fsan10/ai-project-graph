...
在线版本（需要站点访问权限）：[打开 AI 项目图谱](https://ai-project-graph.jz1234da.chatgpt.site)
...
- 使用稳定标识管理节点、连线与层级。
- 支持空格平移画布、滚轮缩放、四向虚线框选、Ctrl 不连续选择、Shift 连续选择和多节点组移动。
- 支持从节点四周直接拖出连线；连线可通过 Ctrl、Shift 或框选批量选择、修改关系和删除。
- 使用单一目标感知右键菜单：画布、节点与连线各自显示匹配操作，兼容桌面右键与移动端长按。
...
自动化测试覆盖领域规则、乐观锁、上下文隔离、证据校验、节点与连线框选、批量关系操作、主题样式、服务端渲染和最终 HTML。提交代码前请确保三个命令全部通过。
...
- Node.js 原生测试运行器
- ChatGPT Sites 部署与 Codex 宿主消息桥接

## 参与贡献

问题报告和功能建议请使用仓库 Issue 模板。代码贡献流程见 [CONTRIBUTING.md](CONTRIBUTING.md)。本项目采用 [MIT License](LICENSE)。
